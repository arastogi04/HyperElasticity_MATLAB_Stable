...
num_load_step=total_load_step; %steps
...
for it=1:total_load_step
    fprintf(file01,'\n\n STEP: %5d',it);
    fprintf('\n\n STEP: %5d    sim time: %6.3f',it,simulation_time(1+it));
    ...
    while (1)
        ...
        [K,R] = getKf(d,ul,xl,connect_list,add_field_e,element_dof,ndofs,sparse_sum);
        %% external force
        ...
        if (1)
            ...
            thisload = load_factor.*force_mag;
        else
            [ul_list,old_ul_list,sMu,dlam,delul_list,free_dofs] = solve_arclength(it,delul_list,ul_list,old_ul_list,K,R,u_increment_bc_dofs,ndofs,count,res,Fef,lambda,dlam,sMu);
            ...
            [ ~ , R ] = getKf(d,ul,xl,connect_list,add_field_e,element_dof,ndofs,sparse_sum);
            
            res =   R(free_dofs) + (lambda+dlam).*Fef(free_dofs);
            [residual]=compute_residual2(res); if (count==1) residual_initial=residual; end;
       
            thisload = (lambda+dlam).*Fef(forced_dofs);
        end
        ...
        if abs(residual)<tol
            ...
            smuu = [smuu sMu];
            dispARC = [dispARC ul_list(forced_dofs)];
            
            loadARC = [loadARC thisload];
...
dispARC = full(dispARC)';
loadARC = loadARC';
hold on
aab = [dispARC*-1 -1*loadARC];
line(aab(:,1),aab(:,2));
scatter(aab(:,1),aab(:,2),'*r');
toc

%% PLOT FIGURE;
if (0); PlotMesh(x_curr, connect_list,fixed_nodes,disp_nodes); end;
